...
namespace EDDiscovery.DB
{
    ...
    public class SystemClass : EDDiscovery2.DB.InMemory.SystemClass
    {
        ...
        public bool Store(SQLiteConnectionED cn, DbTransaction transaction)
        {
            using (DbCommand cmd = cn.CreateCommand("Insert into Systems (name, x, y, z, cr, commandercreate, createdate, commanderupdate, updatedate, status, first_discovered_by, note, id_eddb, population, faction, government_id, allegiance_id, primary_economy_id, security, eddb_updated_at, state, needs_permit, versiondate, id_edsm) values (@name, @x, @y, @z, @cr, @commandercreate, @createdate, @commanderupdate, @updatedate, @status, @Note, @id_eddb, @population, @faction, @government_id, @allegiance_id, @primary_economy_id,  @security, @eddb_updated_at, @state, @needs_permit, datetime('now'),@id_edsm)"))
            {
                ...
                if (id_eddb != 0)
                {
                    ...
                    cmd.AddParameterWithValue("@first_discovered_by", first_discovered_by);
                ...
                {       // override the cmd.
                    cmd.CommandText = "Insert into Systems (name, x, y, z, cr, commandercreate, createdate, commanderupdate, updatedate, status, first_discovered_by, note, versiondate,id_edsm) values (@name, @x, @y, @z, @cr, @commandercreate, @createdate, @commanderupdate, @updatedate, @status, @Note, datetime('now'),@id_edsm)";
                    cmd.AddParameterWithValue("@name", name);
                    cmd.AddParameterWithValue("@x", x);
                    cmd.AddParameterWithValue("@y", y);
                    cmd.AddParameterWithValue("@z", z);
        ...
        public bool Update()
        {
            using (SQLiteConnectionED cn = new SQLiteConnectionED())
            {
                return Update(cn, null);
            }
        }

        public bool Update(SQLiteConnectionED cn, DbTransaction transaction)
        {
            using (DbCommand cmd = cn.CreateCommand("Update Systems set name=@name, x=@x, y=@y, z=@z, cr=@cr, commandercreate=@commandercreate, createdate=@createdate, commanderupdate=@commanderupdate, updatedate=@updatedate, status=@status, first_discovered_by=@first_discovered_by, note=@Note, id_eddb=@id_eddb, population=@population, faction=@faction, government_id=@government_id, allegiance_id=@allegiance_id, primary_economy_id=@primary_economy_id,  security=@security, eddb_updated_at=@eddb_updated_at, state=@state, needs_permit=@needs_permit, versiondate=datetime('now'), id_edsm=@id_edsm where ID=@id",  transaction))
            {
        ...
        public static List<string> GetAllFirstDiscoveryCommanders()
        {
            ...
            try
            {
                using (SQLiteConnectionED cn = new SQLiteConnectionED())
                {
                    using (DbCommand cmd = cn.CreateCommand("SELECT DISTINCT first_discovered_by FROM Systems ORDER BY first_discovered_by ASC"))
                    {
                        using (DbDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                string value = reader[0] == DBNull.Value ? null : (string)reader[0];
                ...
                if (toupdate.Count > 0)
                {
                    ...
                    while (count < toupdate.Count())
                    {
                        using (DbTransaction transaction = cn2.BeginTransaction())
                        {
                            while (count < toupdate.Count())
                            {
                                toupdate[count].Update(cn2, transaction);       // update existing entry..
